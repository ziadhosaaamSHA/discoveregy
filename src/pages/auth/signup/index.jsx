...
// SignUp collects tourist or guide registration details and submits them to auth.
export default function SignUp() {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    const result = await signup(values.firstName + " " + values.lastName, values.email, values.password, userType, extra);
    if (result.success) {
      const role = String(result.user?.type || "").toLowerCase();
      if (role === "guide" || role === "tour guide" || role === "tourguide") {
        navigate("/guide/home");
